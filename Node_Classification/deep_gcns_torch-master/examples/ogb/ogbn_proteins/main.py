...
def multi_evaluate(valid_data_list, dataset, model, evaluator, device):
    ...
    train_pre_final = torch.mean(torch.from_numpy(np.array(train_pre_ordered_list)), dim=0)
    valid_pre_final = torch.mean(torch.from_numpy(np.array(valid_pre_ordered_list)), dim=0)
    test_pre_final = torch.mean(torch.from_numpy(np.array(test_pre_ordered_list)), dim=0)
...
def main():
    ...
    nf_path = "home/lbalbi/embedding_CC_wang2vec.pt"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if valid_result > results['highest_valid']:
            ...
            print("{}{}valid_best".format(args.model_save_path, sub_dir))
            torch.save({'epoch': epoch,'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict()},  "{}{}valid_best".format(args.model_save_path, sub_dir))
