...
def run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, n_running):
    ...
    import sklearn
    acc = sklearn.metrics.accuracy_score(torch.cat([torch.ones(len(final_pred//2)), torch.zeros(len(final_pred//2))],dim=0),final_pred)

    return best_val_score, final_test_score, acc
...
def main():
    ...
    argparser.add_argument("--K1", action="store")
    ...
    if args.K1:
       graph.ndata["feat"] = torch.load("{}".format(args.K1))
       n_node_feats = graph.ndata["feat"].shape[-1]
    ...
    # run
    val_scores, test_scores, test_acc = [], [], []

    for i in range(args.n_runs):
        print("Running", i)
        seed(args.seed + i)
        val_score, test_score , acc = run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, i + 1)
    ...
    print(f"Average test accuracy: {np.mean(test_acc)} ± {np.std(test_acc)}")
