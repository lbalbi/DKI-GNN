...
def main(args):
    ...
    graph.ndata["labels"] = dataset.labels.float()
    graph.ndata["feat"] = torch.load("{}".format(embedding)).to("cpu")
    ...
    for epoch in range(1, args["num_epochs"] + 1):
        ...
        loss, train_score = run_a_train_epoch(
            graph.to(args["device"]), splitted_idx["train"], model, criterion, optimizer, evaluator
        )
        ...
        train_score, val_score, test_score = run_an_eval_epoch(
            graph.to(args["device"]), splitted_idx, model, evaluator
        )
