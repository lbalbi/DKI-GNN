...
def load_data(dataset, embedding):
    ...
    graph, labels = data[0]
    print(graph)
    graph.ndata["feat"] = torch.load(embedding)
...
def main():
    ...
    argparser.add_argument(
        "--n-runs", type=int, default=2, help="running times"
    )
    argparser.add_argument(
    ...
    print("Loading data")
    graph, labels, train_idx, val_idx, test_idx, evaluator = load_data(dataset, args.K1)
