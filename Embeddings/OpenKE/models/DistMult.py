...
class DistMult(Model):
	...
	def embedding_def(self):
		...
		self.ent_embeddings = tf.get_variable(name = "ent_embeddings", shape = [config.entTotal, config.hidden_size], initializer = tf2.initializers.GlorotUniform())
		self.rel_embeddings = tf.get_variable(name = "rel_embeddings", shape = [config.relTotal, config.hidden_size], initializer = tf2.initializers.GlorotUniform())
