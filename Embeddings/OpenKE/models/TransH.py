...
class TransH(Model):
	...
	def embedding_def(self):
		...
		self.ent_embeddings = tf.get_variable(name = "ent_embeddings", shape = [config.entTotal, config.hidden_size], initializer = tf2.initializers.GlorotNormal())
		self.rel_embeddings = tf.get_variable(name = "rel_embeddings", shape = [config.relTotal, config.hidden_size], initializer = tf2.initializers.GlorotNormal())
		self.normal_vectors = tf.get_variable(name = "normal_vectors", shape = [config.relTotal, config.hidden_size], initializer = tf2.initializers.GlorotNormal())
