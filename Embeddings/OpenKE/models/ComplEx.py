...
class ComplEx(Model):
	...
	def embedding_def(self):
		...
		self.ent1_embeddings = tf.get_variable(name = "ent1_embeddings", shape = [config.entTotal, config.hidden_size], initializer = tf2.initializers.GlorotUniform())
		self.rel1_embeddings = tf.get_variable(name = "rel1_embeddings", shape = [config.relTotal, config.hidden_size], initializer = tf2.initializers.GlorotUniform())
		self.ent2_embeddings = tf.get_variable(name = "ent2_embeddings", shape = [config.entTotal, config.hidden_size], initializer = tf2.initializers.GlorotUniform())
		self.rel2_embeddings = tf.get_variable(name = "rel2_embeddings", shape = [config.relTotal, config.hidden_size], initializer = tf2.initializers.GlorotUniform())
